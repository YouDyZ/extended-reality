    ...
    doRagdoll() {
        ...
        if (game.isPedInAnyVehicle(this.player.scriptID, false)) {
            ...
            if (this.ragdoll === false) {
                if (
                    game.getWeaponClipSize(currentWeapon) > 0 &&
                    !game.isPedJumping(this.player.scriptID) &&
                    !game.isPlayerClimbing(this.player.scriptID) &&
                    !game.isPedFalling(this.player.scriptID)
                ) {
                    return;
                }
            }
